...
def create_codebook(obj_id, config, model_func, obj_model_file,
        intrinsic, device, obj_diameter, codebook_dir):
...
def OVE6D_mask_full_pose(model_func, obj_depth, obj_mask, obj_codebook, cam_K, config, obj_renderer, device):
    ...
    # Inital translation estimate given object crop and cropped depth.
    ...
    # View point estimate and inplane rotation regression.
    ...
    for idx, (obj_est_R, obj_est_score) in enumerate(zip(estimated_R, estimated_scores)):
        ...
        # ICP on current frame cropped depth AND final top K in plane rotated hypothesis.
        if config.USE_ICP:
            icp_timer = time.time()
